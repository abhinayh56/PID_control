 ...
 *
 * @details
 * This file defines the @ref control_system::derivative::D_LPF_1 class,
 * which implements a discrete-time derivative controller with a
 * first-order low-pass filter.
 *
 * The controller uses a configurable derivative gain, sampling time,
 * and filter cutoff frequency. The previous control error and controller
 * output are maintained as internal state variables.
 *
...
namespace control_system
{
    ...
    namespace derivative
    {
        ...
        /**
         * @class D_LPF_1
         * @brief Derivative controller with a first-order low-pass filter.
         *
         * @details
         * The D_LPF_1 class implements a discrete-time derivative
         * controller with a first-order low-pass filter applied to the
         * derivative action.
         *
         * The controller uses the sampling time, derivative gain, and
         * filter cutoff frequency to calculate the derivative contribution.
         *
         * The previous control error and controller output are maintained
         * as internal state, and the controller output is limited by the
         ...
         * @par Example
         * @code{.cpp}
         * control_system::derivative::D_LPF_1<double> controller;
         *
         * controller.init(
         *     0.001,
         *     Kd,
        ...
        public:
             ...
             * filter cutoff frequency, and maximum output limit.
             */
            void set_param(T dt_, T Kd_, T fc_, T u_max_);

            /**
             * @brief Computes the filtered derivative control output.
             *
             * @param[in] x_0 Reference or desired value.
             * @param[in] x Current or measured value.
             *
             * @return Filtered derivative controller output limited by
             * the configured maximum output.
             *
             * @details
             * Calculates the derivative contribution from the change in
             * control error and applies the configured first-order
             * low-pass filtering.
             */
            T update(T x_0, T x);
             ...
             *
             * @details
             * Resets the stored control error, previous controller output,
             * and startup state.
             */
            void reset();
        ...
        private:
            ...
            /**
             * @brief Time constant of the first-order low-pass filter.
             */
            T tau = 0.0;
...
#include "../../src/derivative/d_lpf_1.tpp"
    }
}
